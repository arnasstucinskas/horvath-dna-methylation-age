...
# anti.trafo function: The inverse of the 'trafo' function. It reverts 
# the transformed age values back to their original scale. This function 
# is useful for interpreting results after analyses have been conducted 
...
if (fastImputation | nSamples==1) {
  ...
  if (max(noMissingPerSample,na.rm=TRUE) > 0 &
    ...
    datMethUsed[i,selectMissing1] <-
      as.numeric(probeAnnotation21kdatMethUsed$goldstandard2[selectMissing1])
    } # end of for loop
...
if ( !is.null( meanXchromosome) ){  
  if ( length( meanXchromosome)==dim(datout)[[1]] ){
    ...
    datout <- data.frame(datout, predictedGender = predictedGender,
